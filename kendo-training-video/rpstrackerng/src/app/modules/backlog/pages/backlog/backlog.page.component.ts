...
import { PresetType, PtItemType } from "src/app/core/models/domain/types";
...
export class BacklogPageComponent implements OnInit {
        ...
        (result) => {
          if (typeof result === "object") {
            if (this.store.value.currentUser) {
              this.backlogService
                .addNewPtItem(result, this.store.value.currentUser)
                .then((nextItem) => {
  ...
  public getIndicatorImageFromType(ptItemType: PtItemType): string {
    return ItemType.imageResFromType(ptItemType);
  }

  public toBadgeClass(ptItemType: PtItemType): string {
    return `badge ${this.getIndicatorClassFromType(ptItemType)}`;
  }

  private getIndicatorClassFromType(ptItemType: PtItemType): string {
    return ItemType.indicatorClassFromType(ptItemType);
  }
}
